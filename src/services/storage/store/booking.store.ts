...
export const useBookingStore = create<BookingState>((set, get) => ({
  ...
  createBooking: async (data: any) => {
    console.log("🟡 [STORE] createBooking called with data:", data);

    set({ isLoading: true, error: null });

    try {
      console.log("🟡 [STORE] Calling bookingService.createBooking...");

      const booking = await bookingService.createBooking(data);

      console.log("🟢 [STORE] Booking API response:", booking);
      ...
      console.log("🔴 [STORE] Booking error:", error);
      console.log("🔴 [STORE] Error response:", error?.response);
      console.log("🔴 [STORE] Error data:", error?.response?.data);
  ...
  extendTrip: async (bookingId: string, newEndDate: string) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to extend trip";
      set({ error: errorMessage, isLoading: false });
      throw error;
    }
  },
